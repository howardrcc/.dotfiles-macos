...
- Homebrew 5.2+ removed `brew bundle --cleanup` entirely — use `cleanup = "none"` in `homebrew.onActivation` to suppress the deprecation error. Manual cleanup: `brew bundle cleanup --force --file=/dev/stdin < <(nix eval ...)`
